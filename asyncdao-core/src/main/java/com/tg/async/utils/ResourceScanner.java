...
public class ResourceScanner {
	...
	private static void getByPackage(String packageName, Set<String> contents, String fileSuffix,
			TransformHandler handler) throws Exception {
	...
	private static void findFileInJarWithinJar(String filePath, String packagePath, Set<String> classes)
			throws IOException {
		URL url = new URL("jar", null, 0, filePath);
		URLConnection con = url.openConnection();
		if (con instanceof JarURLConnection) {
	...
	private static void getFromFile(String filePath, Set<String> contents, String fileSuffix, TransformHandler handler)
			throws Exception {
	...
	private static String getWholeClassName(String filePath) {
		if (filePath.indexOf("classes/") != -1) {
			return filePath.substring(filePath.indexOf("classes/") + "classes/".length(), filePath.indexOf(".class"))
					.replaceAll("/", ".");
		}
	...
	static class WholeClassNameHandler implements TransformHandler {
		...
		public String handle(String s) {
			if (s.indexOf("classes/") != -1) {
				return s.substring(s.indexOf("classes/") + "classes/".length(), s.indexOf(".class")).replaceAll("/",
						".");
			}
