...
public class DataConverter {
	...
	public static <T> T rowDataToObject(RowData rowData, Class<T> clazz, ModelMap resultMap, List<String> columnNames)
			throws Exception {
		Iterator<Object> iterable = rowData.iterator();
		...
		if (columnNames.size() == 1 && (clazz.getClassLoader() == null)
			...
			if (item == null) {
				...
				throw new ParseException(String.format(
						"data type you want convert is %s ,but database return type is %s ,just change it", clazz,
						item.getClass()));
			} else {
				return (T) item;
			}
