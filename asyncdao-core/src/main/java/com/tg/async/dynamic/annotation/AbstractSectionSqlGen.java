...
public abstract class AbstractSectionSqlGen implements SectionSqlGen {
	...
	protected SqlNode parse(ConditionWrap condition) {
		if (sqlMode.equals(SqlMode.COMMON)) {
			if (!condition.getCriterion().inCriterion()) {
				...
				String sql = condition.getAttach().toString() + " "
						+ getColumn(condition.getField(), condition.getColumn()) + " "
						+ condition.getCriterion().getCriterion();
				return new MixedSqlNode(
						Arrays.asList(new StaticTextSqlNode(sql), generateForEachNode(condition.getOgnlParam())));
			}
		} else {
			if (!condition.getCriterion().inCriterion()) {
	...
	protected String simpleCondition(ConditionWrap condition) {
		...
		stringBuilder.append(condition.getAttach().toString()).append(" ")
				.append(getColumn(condition.getField(), condition.getColumn())).append(" ")
				.append(condition.getCriterion().getCriterion()).append(" #{").append(condition.getOgnlParam())
				.append("}");
