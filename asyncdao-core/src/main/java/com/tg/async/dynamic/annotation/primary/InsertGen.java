...
public class InsertGen extends AbstractSqlGen {
	...
	protected SqlNode generateBaseSql() {
		...
		List<SqlNode> mixedSqlNode = Arrays.asList(new StaticTextSqlNode(data),
	...
	public MappedStatement generate() {
		...
		MappedStatement mappedStatement = new MappedStatement.Builder(buildKey(), new DynamicSqlSource(rootSqlNode),
				sqlType()).keyGenerator(String.valueOf(insert.useGeneratedKeys())).keyProperty(insert.keyProperty())
						.build();
