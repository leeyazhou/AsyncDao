...
public class FlatParamWhereSqlGen extends AbstractSectionSqlGen {
	...
	public SqlNode generateSql() {
		...
		for (int i = 0; i < parameters.length - 1; i++) {
			...
			if (condition == null) {
				if (!parameters[i].isAnnotationPresent(Limit.class)
						&& !parameters[i].isAnnotationPresent(OffSet.class)) {
					nodes.add(new StaticTextSqlNode(simpleCondition(ConditionWrap.empty(paramName))));
				}
				continue;
			}
			ConditionWrap conditionWrap = ConditionWrap.builder().attach(condition.attach()).column(condition.column())
					.criterion(condition.criterion()).field(paramName).ognlParam(paramName).build();
