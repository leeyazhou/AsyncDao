...
public class XMLMapperBuilder {
	...
	private ColumnMapping buildResultMapping(XNode node) {
		return new ColumnMapping(node.getStringAttribute("column"), node.getStringAttribute("property"));
	}
	...
	private void parseStatement(XNode node) {
		...
		MappedStatement mappedStatement = new MappedStatement.Builder(id, new DynamicSqlSource(rootSqlNode), mode)
				.keyGenerator(useGeneratedKeys).keyProperty(keyProperty).resultType(resultType).resultMap(resultMap)
				.build();
	...
	private class ForEachHandler implements NodeHandler {
		...
		public void handleNode(XNode nodeToHandle, List<SqlNode> targetContents) {
			...
			ForEachSqlNode forEachSqlNode = new ForEachSqlNode(mixedSqlNode, collection, index, item, open, close,
					separator);
